<template>
    <div>
        <div class="container">
            ...
            <div> {{$store.state.AuthModule.registerError}}</div>
        </div>
    </div>
</template>
...
export default {
    ...
    computed: {
        isValid() {
            ...
            if (regexPatternEmail.test(this.email) && this.password !== "" && this.address !== "" 
            && regexPatternFullName.test(this.full_name) && this.checkCaptcha === true)
                return false;
            return true;
        },
        validateEmail(){
